-- BSD VIP Clients
-- Approved for controlled production test on 2026-09-17.
...
create index if not exists vip_sessions_token_idx on public.vip_sessions(token_hash);
create index if not exists vip_sessions_account_created_idx on public.vip_sessions(vip_account_id, created_at desc);
...
create index if not exists vip_login_attempts_guard_idx on public.vip_login_attempts(username, ip_hash, attempted_at desc);
create index if not exists vip_login_attempts_cleanup_idx on public.vip_login_attempts(attempted_at);
create index if not exists vip_login_attempts_failed_idx on public.vip_login_attempts(username, ip_hash, attempted_at desc) where success = false;
...
create index if not exists vip_publications_enabled_idx on public.vip_business_publications(enabled, published_at desc);
...
create index if not exists vip_inquiries_status_created_idx on public.vip_inquiries(status, created_at desc);
...
declare f public.business_sale_files%rowtype;
begin
  if new.anonymous_file_id is null then
    if new.enabled then raise exception 'VIP publication requires an approved anonymous file'; end if;
    return new;
  end if;
  select * into f from public.business_sale_files where id = new.anonymous_file_id;
  if not found then raise exception 'Anonymous file not found'; end if;
  if f.business_id <> new.business_id then raise exception 'Anonymous file belongs to a different business'; end if;
  if f.status <> 'active' then raise exception 'Anonymous file is not active'; end if;
  if f.confidentiality_level <> 1 then raise exception 'Only confidentiality level 1 files may be published to VIP'; end if;
  if coalesce(f.document_type, '') <> 'anonymous_summary' then raise exception 'Only document_type anonymous_summary may be published to VIP'; end if;
  return new;
end;
$$;
...
create trigger trg_vip_validate_anonymous_file before insert or update on public.vip_business_publications for each row execute function public.vip_validate_anonymous_file();

create or replace function public.vip_touch_updated_at() returns trigger language plpgsql as $$ begin new.updated_at = now(); return new; end; $$;
drop trigger if exists trg_vip_accounts_touch on public.vip_accounts;
create trigger trg_vip_accounts_touch before update on public.vip_accounts for each row execute function public.vip_touch_updated_at();
drop trigger if exists trg_vip_publications_touch on public.vip_business_publications;
create trigger trg_vip_publications_touch before update on public.vip_business_publications for each row execute function public.vip_touch_updated_at();
drop trigger if exists trg_vip_interests_touch on public.vip_interests;
create trigger trg_vip_interests_touch before update on public.vip_interests for each row execute function public.vip_touch_updated_at();

create or replace function public.vip_cleanup_login_attempts() returns void language sql security definer set search_path = public as $$ delete from public.vip_login_attempts where attempted_at < now() - interval '24 hours'; $$;
revoke all on function public.vip_cleanup_login_attempts() from public;
