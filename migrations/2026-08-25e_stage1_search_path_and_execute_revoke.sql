...
CREATE OR REPLACE FUNCTION public.trg_assign_business_number()
...
BEGIN
  ...
  NEW.business_number_running := nextval('public.business_number_seq');
  NEW.business_number := 'BSD-B-' || yy || mm || '-' || NEW.business_number_running;
  RETURN NEW;
END;
$$;

CREATE OR REPLACE FUNCTION public.trg_protect_business_number()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER
SET search_path = pg_catalog AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.trg_assign_client_number()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER
SET search_path = pg_catalog AS $$
DECLARE
  il_ts timestamptz;
...
CREATE OR REPLACE FUNCTION public.trg_protect_client_number()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER
SET search_path = pg_catalog AS $$
BEGIN
...
REVOKE EXECUTE ON FUNCTION public.trg_assign_business_number() FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.trg_protect_business_number() FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.trg_assign_client_number() FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.trg_protect_client_number() FROM PUBLIC, anon, authenticated;
